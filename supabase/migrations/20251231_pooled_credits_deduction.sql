-- Migration: Pooled Credit Deduction
-- Purpose: Implement a function that deducts credits from an organization's pool if an org context is provided,
--          otherwise falls back to personal credits. Strictly fails if the chosen pool is insufficient.
...
BEGIN
  ...
  IF p_clerk_org_id IS NOT NULL THEN
    -- Verify user has an active seat in this organization
    SELECT organization_subscription_id INTO v_org_sub_id
    ...
    -- Check organization pooled balance
    SELECT total_credits, used_credits INTO v_total_credits, v_used_credits
    ...
    IF (v_total_credits - v_used_credits) < p_amount THEN
      RETURN FALSE; -- Insufficient pooled credits
    END IF;
    ...
    VALUES (
      ...
      p_description, 
      p_reference_id,
      jsonb_build_object('clerk_org_id', p_clerk_org_id, 'pool', 'organization')
    );
